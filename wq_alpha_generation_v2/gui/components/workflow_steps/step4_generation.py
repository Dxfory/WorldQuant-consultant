...
Generate a valid FASTEXPR expression that uses operator(data_field, parameters) syntax."""
                    ...
                    if available_operators:
                        ...
                        with operator_pool_lock:
                            ...
                            # ------------------------------------------------------------------
                            # HARD SAFETY FILTER FOR STEP 4 OPERATOR SELECTION
                            # Step 4 must not select operators that require special parameters
                            # such as lookback, group/category, or reduce structures.
                            # Keep this local because Step 4 has its own diverse-operator path.
                            # ------------------------------------------------------------------
                            safe_operator_allowlist = {
                                "rank",
                                "normalize",
                                "winsorize",
                                "zscore",
                            ...
                            def _step4_operator_name(op):
                                if isinstance(op, dict):
                                    return str(op.get("name", "")).strip().lower()
                                return str(getattr(op, "name", "")).strip().lower()

                            original_available_indices = list(available_indices)

                            safe_available_indices = []
                            for idx in original_available_indices:
                                if idx >= len(available_operators):
                                    continue

                                op_name = _step4_operator_name(available_operators[idx])
                                if not op_name:
                                    continue

                                if op_name in unsafe_operator_blocklist:
                                    continue

                                if op_name not in safe_operator_allowlist:
                                    continue

                                safe_available_indices.append(idx)

                            if safe_available_indices:
                                logger.debug(
                                    f"[Step 4] Slot {primary_slot_id+1}: "
                                    f"Filtered operators {len(original_available_indices)} -> "
                                    f"{len(safe_available_indices)} safe operators"
                                )
                                available_indices = safe_available_indices
                            else:
                                logger.warning(
                                    f"[Step 4] Slot {primary_slot_id+1}: "
                                    f"No safe operators after filtering; keeping original operator pool"
                                )
                            ...
                            if len(candidate_indices) < 2:
                                ...
                                operator_usage_count.clear()
                                candidate_indices = list(available_indices)
                                if len(candidate_indices) < 2:
