...
print("[ollama_manager]   ?ollama will be imported lazily when first used", flush=True)
    ...
    def _check_availability(self) -> bool:
        ...
        try:
            ...
            if "qwen" in self.model.lower() or "coder" in self.model.lower():
                preferred = ["qwen2.5-coder:7b", "qwen2.5-coder:7b", "qwen2.5-coder:32b", self.model]
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if 'model' in error_msg.lower() or 'not found' in error_msg.lower():
                    ...
                    preferred = []
                    if "qwen" in self.model.lower() or "coder" in self.model.lower():
                        preferred = ["qwen2.5-coder:7b", "qwen2.5-coder:7b", "qwen2.5-coder:32b"]
                    alternative = self._find_available_model(preferred)
...
13. Field IDs are case-sensitive and must match EXACTLY from the available fields list"""
        ...
        if available_operators:
            ...
            safe_ollama_operator_allowlist = {
                "rank",
                "normalize",
                "winsorize",
                "zscore",
            ...
            unsafe_ollama_operator_blocklist = {
                "add",
                "subtract",
                "multiply",
                "divide",
            ...
            for i, op in enumerate(selected_operators):
                ...
                if name:
                    op_name_lower = str(name).strip().lower()

                    if op_name_lower in unsafe_ollama_operator_blocklist:
                        logger.debug(f"Skipping unsafe Ollama operator: {op_name_lower}")
                        continue

                    if op_name_lower not in safe_ollama_operator_allowlist:
                        logger.debug(f"Skipping non-allowlisted Ollama operator: {op_name_lower}")
                        continue
            ...
            if operator_list:
                ...
                user_prompt += "\n\n🚫 CRITICAL UNSUPPORTED OPERATOR RULES:"
                user_prompt += "\n- Never invent operators."
                user_prompt += "\n- Never create ts_ versions of ordinary operators."
                user_prompt += "\n- Do NOT use ts_add, ts_subtract, ts_multiply, ts_divide, ts_log, or ts_sqrt."
                user_prompt += "\n- Use ONLY operators that appear in the AVAILABLE OPERATORS list."
                user_prompt += "\n- If an operator is not explicitly listed, it is forbidden."
                user_prompt += "\n- Use placeholder names exactly: OPERATOR1, OPERATOR2, DATA_FIELD1, DATA_FIELD2."
                user_prompt += "\n- Do not output actual operator names as JSON keys, such as ADD, AND, NOT, VEC_AVG."            
                user_prompt += "\n- Do not select event/vector/timestamp fields for DATA_FIELD placeholders."
                user_prompt += "\n- Use ordinary numeric matrix/scalar fields unless the template explicitly uses a vector/event-compatible operator." 
        ...
        if use_placeholder_fields and available_fields:
            ...
            # Filter unsafe event/news/timestamp fields before showing fields to Ollama.
            # This prevents Ollama from selecting fields that repeatedly cause errors like:
            ...
            def _is_safe_ollama_field(field) -> bool:
                """Keep Ollama from seeing event/vector fields in generic templates."""
                if isinstance(field, dict):
                    ...
                    if any(t in {"event", "events", "vector", "vectors"} for t in type_values):
                        return False
                ...
                return not any(bad in text for bad in bad_field_keywords)

            safe_field_items = [
                (idx, field)
                for idx, field in enumerate(available_fields)
                if _is_safe_ollama_field(field)
            ]

            if safe_field_items:
                logger.debug(
                    f"Filtered Ollama field pool: {len(available_fields)} -> {len(safe_field_items)} safe fields"
                )
            else:
                logger.warning(
                    "Safe Ollama field filter removed all fields; falling back to original available_fields."
                )
                safe_field_items = list(enumerate(available_fields))

            # Randomly select a subset of safe fields (between 15-30 fields) to show Ollama.
            num_fields_to_show = min(random.randint(15, 30), len(safe_field_items))
            selected_field_items = random.sample(safe_field_items, num_fields_to_show)

            # Keep original indices so later mapping remains consistent.
            selected_field_indices = [idx for idx, field in selected_field_items]
            selected_fields_to_show = [field for idx, field in selected_field_items]
