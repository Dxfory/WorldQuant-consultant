...
class SimulatorTester:
    ...
    def _has_single_argument_call(self, expr: str, op_name: str) -> bool:
        """Roughly detect calls like add(x), subtract(x), multiply(x), divide(x)."""
        pattern = rf"\b{re.escape(op_name)}\s*\(([^()]*)\)"
        for match in re.finditer(pattern, expr, flags=re.IGNORECASE):
            inside = match.group(1).strip()
            if inside and "," not in inside:
                return True
        return False
    
    def _iter_call_arg_strings(self, expr: str, op_name: str):
        """Yield raw argument strings for calls like op_name(...), supporting nested parentheses."""
        pattern = r"\b" + re.escape(op_name) + r"\s*\("
        for match in re.finditer(pattern, expr, flags=re.IGNORECASE):
            start = match.end()
            depth = 1
            i = start
    ...
    def _split_top_level_args(self, arg_string: str) -> list:
    ...
    def _iter_call_spans(self, expr: str, op_name: str):
        """Yield (start, end, raw_args) for calls like op_name(...), supporting nested parentheses."""
        pattern = r"\b" + re.escape(op_name) + r"\s*\("
        for match in re.finditer(pattern, expr, flags=re.IGNORECASE):
            ...
            if depth == 0:
                yield match.start(), i, expr[start_args:i - 1]

    def _repair_single_arg_binary_ops(self, expr: str) -> str:
        """Repair binary arithmetic operators accidentally generated with one argument.

        Examples:
            add(x)      -> add(x, 0)
            subtract(x) -> subtract(x, 0)
        ...
        # Multiple passes handle nested or repeated expressions safely.
        for _ in range(3):
            changed = False

            for op_name, neutral_arg in neutral_args.items():
                spans = list(self._iter_call_spans(result, op_name))

                for start, end, inside in reversed(spans):
                    args = self._split_top_level_args(inside)

                    if len(args) == 1:
                        repaired_call = f"{op_name}({args[0]}, {neutral_arg})"
                        result = result[:start] + repaired_call + result[end:]
                        changed = True
                        changed_any = True
        ...
        if changed_any:
            logger.info(
                f"Repaired single-argument binary operator(s): "
                f"{str(expr)[:120]} -> {result[:120]}"
            )

        return result
    
    def _get_db_path(self) -> str:
        """Best-effort database path lookup for learned event-input fields."""
        if self.template_generator is not None:
            if hasattr(self.template_generator, "db_path"):
                return self.template_generator.db_path
            validator = getattr(self.template_generator, "template_validator", None)
            if validator is not None and hasattr(validator, "db_path"):
                return validator.db_path
        return "generation_two_backtests.db"

    def _extract_candidate_field_ids(self, expr: str) -> set:
        """Extract likely field ids from a FASTEXPR expression."""
        if not expr:
            return set()

        tokens = re.findall(r"\b[a-z][a-z0-9_]{3,}\b", str(expr).lower())
        excluded = {
            ...
            "and", "or", "not", "true", "false", "nan", "filter", "to_nan",
            "is_nan", "group_neutralize", "group_rank", "group_zscore",
    ...
    def _load_event_input_fields_from_db(self) -> set:
        """Load DB-marked event/vector fields for pre-submission rejection."""
        try:
            import sqlite3
            ...
            logger.debug(f"Could not load event-input field list from DB: {e}")
            return set()

    def _should_reject_before_simulation(self, expr: str, region: str = None, delay: int = None) -> tuple:
        """Hard pre-simulation validator.

        Reject expressions that are very likely to fail before spending a
        WorldQuant Brain simulation request.
        """
        ...
        for keyword in bad_keywords:
            if keyword.lower() in expr_lower:
                return True, f"blocked keyword before simulation: {keyword}"

        db_event_fields = self._load_event_input_fields_from_db()
        if db_event_fields:
            used_fields = self._extract_candidate_field_ids(expr_lower)
            event_hits = sorted(used_fields.intersection(db_event_fields))
            if event_hits:
                return True, f"blocked DB-marked event input field before simulation: {event_hits[:5]}"
        ...
        for op_name in bad_operator_names:
            if re.search(r"\b" + re.escape(op_name) + r"\s*\(", expr_lower):
                return True, f"blocked unsupported operator before simulation: {op_name}"
        unary_ops = [
            "log",
            "sqrt",
            "abs",
        ...
        for op_name in unary_ops:
            for inside in self._iter_call_arg_strings(expr_lower, op_name):
                args = self._split_top_level_args(inside)
                if len(args) > 1:
                    return True, f"{op_name} appears to have too many inputs"
        ...
        for op_name in binary_like_ops:
            if self._has_single_argument_call(expr_lower, op_name):
                return True, f"{op_name} appears to have only one input"
    ...
    def submit_simulation(
        ...
        try:
            ...
            if not self.sess.cookies:
                ...
                # Repair common LLM-generated parameter-count mistakes before hard rejection.
            template = self._repair_single_arg_binary_ops(template)

            # Final hard guard: reject binary operators that still have one argument after repair.
            for _bad_binary_op in ["add", "subtract", "multiply", "divide"]:
                if self._has_single_argument_call(str(template).lower(), _bad_binary_op):
                    logger.warning(
                        f"Rejected after repair: {_bad_binary_op} still appears to have only one input | "
                        f"template={str(template)[:200]}"
                    )
                    return None
    
            should_reject, reject_reason = self._should_reject_before_simulation(
                template,
                region=region,
            ...
            if should_reject:
                logger.warning(
                    f"Rejected before simulation: {reject_reason} | template={str(template)[:200]}"
                )
                return None
