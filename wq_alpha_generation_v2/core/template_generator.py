...
class TemplateGenerator:
    ...
    # ------------------------------------------------------------------
    # Field-safety helpers
    # ------------------------------------------------------------------
    _UNSAFE_FIELD_KEYWORDS = {
        ...
        "record_end_time",
        "record_start_time",
        "news31_amertimestamp",
        "news31_apactimestamp",
        "news_timestamp",
        "arrival_time",

        # Text / categorical / identifier fields
        "_iso",
        "iso",
        "crncy",
        "currency",
    ...
    def _field_id(self, field: Dict) -> str:
        """Return the canonical field id from a field dictionary."""
    ...
    def _field_text(self, field: Dict) -> str:
        """Flatten field metadata for conservative keyword/type checks."""
        if not isinstance(field, dict):
    ...
    def _metadata_marks_event_input(self, field: Dict) -> bool:
        """Detect event/vector fields from API metadata."""
        if not isinstance(field, dict):
            return False

        truthy = {"1", "true", "yes", "y"}
        for flag_key in ("is_event_input", "isEventInput", "eventInput", "isEvent"):
            if str(field.get(flag_key, "")).strip().lower() in truthy:
                return True

        type_values = []
        for type_key in ("type", "fieldType", "dataType", "inputType", "outputType"):
            value = field.get(type_key, "")
            if isinstance(value, dict):
                type_values.extend(str(v).lower() for v in value.values())
            else:
                type_values.append(str(value).lower())

        if any(t in self._UNSAFE_FIELD_TYPES for t in type_values):
            return True

        text = self._field_text(field)
        return any(keyword in text for keyword in self._UNSAFE_FIELD_KEYWORDS)

    def _load_db_event_input_fields(self, region: str = None, delay: int = None) -> set:
        """Load field ids that have been stored or learned as event inputs."""
        import sqlite3
        ...
        try:
            ...
            if "field_types" not in tables:
                conn.close()
                return set()

            cols = [row[1] for row in cursor.execute("PRAGMA table_info(field_types)").fetchall()]
            field_col = "field_id" if "field_id" in cols else ("id" if "id" in cols else None)
            if not field_col:
                conn.close()
                return set()

            where_parts = []
            if "is_event_input" in cols:
                where_parts.append("COALESCE(is_event_input, 0) = 1")
            if "field_type" in cols:
                where_parts.append("LOWER(COALESCE(field_type, '')) IN ('event', 'events', 'vector', 'vectors')")
            if "category" in cols:
                where_parts.append("LOWER(COALESCE(category, '')) LIKE '%event%'")
            if "field_data" in cols:
                where_parts.append("LOWER(COALESCE(field_data, '')) LIKE '%\"event%'")
                where_parts.append("LOWER(COALESCE(field_data, '')) LIKE '%\"vector%'")

            if not where_parts:
                conn.close()
                return set()

            sql = f"SELECT DISTINCT {field_col} FROM field_types WHERE " + " OR ".join(where_parts)
            result = {
                str(row[0]).lower()
            ...
            logger.debug(f"Failed to load DB event-input fields: {e}")
            return set()

    def _is_safe_data_field(self, field: Dict, db_event_input_fields: set = None) -> bool:
        """Return False for fields that should not be used in generic templates."""
        field_id = self._field_id(field).lower()
        db_event_input_fields = db_event_input_fields or set()

        if field_id and field_id in db_event_input_fields:
            logger.debug(f"Field blocked by DB event-input list: {field_id}")
            return False

        if self._metadata_marks_event_input(field):
            return False

        return True

    def _filter_safe_data_fields(self, fields: List[Dict], region: str = None, delay: int = None) -> List[Dict]:
        """Filter data fields before generation/placeholder replacement."""
        if not fields:
            return []

        db_event_input_fields = self._load_db_event_input_fields(region=region, delay=delay)
        safe_fields = [
            field for field in fields
            if self._is_safe_data_field(field, db_event_input_fields=db_event_input_fields)
        ]

        logger.debug(
            f"Safe data field filter for {region or 'N/A'} delay={delay}: "
            f"{len(fields)} -> {len(safe_fields)} "
            f"(db_event_inputs={len(db_event_input_fields)})"
        )

        return safe_fields

    def _store_field_types(self, fields: List[Dict], region: str, delay: int):
        """Store field type information including conservative event-input detection."""
        try:
            import sqlite3
            import json
            from ..storage.backtest_storage import BacktestStorage
            ...
            for field in fields:
                ...
                is_event_input = self._metadata_marks_event_input(field)
    ...
    def generate_templates_for_region(
        ...
        raw_field_count = len(data_fields) if data_fields else 0
        data_fields = self._filter_safe_data_fields(data_fields, region=region, delay=1)

        if not data_fields:
            logger.warning(
                f"No safe non-event data fields available for {region} "
                f"after filtering {raw_field_count} raw fields"
            )
            return templates
...
Generate a valid FASTEXPR expression that uses operator(data_field, parameters) syntax."""
                    ...
                    if template and selected_fields:
                        template = self._replace_field_placeholders(template, selected_fields, region=region)
    ...
    def _select_fields_v2(self, data_fields: List[Dict], num_fields: int = 3) -> List[int]:
        """V2 Approach: Select safer field indices.

        The returned indices still refer to the original data_fields list, so
        downstream mapping behavior is unchanged.  The only change is that
        event/vector/timestamp fields are not offered to generic FASTEXPR
        templates.
        """
        import random
        ...
        db_event_input_fields = self._load_db_event_input_fields()
        safe_indices = []
        preferred_indices = []

        for idx, field in enumerate(data_fields):
            if not self._is_safe_data_field(field, db_event_input_fields=db_event_input_fields):
                continue

            safe_indices.append(idx)
            text = self._field_text(field)
            if any(good in text for good in preferred_keywords):
                preferred_indices.append(idx)

        candidate_indices = preferred_indices if len(preferred_indices) >= num_fields else safe_indices

        if not candidate_indices:
            logger.warning(
                "Safe field filter removed all fields; returning no field indices "
                "instead of falling back to event/vector fields."
            )
            return []

        num_fields = min(num_fields, len(candidate_indices))
        selected_indices = random.sample(candidate_indices, num_fields)

        logger.debug(
            f"Selected safe field indices: {selected_indices} "
            f"(safe={len(safe_indices)}, preferred={len(preferred_indices)}, "
            f"total={len(data_fields)}, db_event_inputs={len(db_event_input_fields)})"
        )

        return selected_indices

    def _operator_name(self, op: Dict) -> str:
        """Return canonical operator name."""
        if not isinstance(op, dict):
            return str(op or "")
        return str(op.get("name") or op.get("id") or "").strip()

    def _operator_input_count(self, op: Dict) -> int:
        """Best-effort extraction of operator input count from metadata."""
        import re

        if not isinstance(op, dict):
            return 0

        for key in ("input_count", "inputCount", "num_inputs", "numInputs", "arity"):
            value = op.get(key)
        ...
        text_parts = []
        for key in ("definition", "description", "documentation", "usage"):
            value = op.get(key, "")
            if isinstance(value, dict):
        ...
        match = re.search(r"takes\s+(\d+)\s+input", text)
        if match:
            return int(match.group(1))

        match = re.search(r"(\d+)\s+input", text)
        if match:
            return int(match.group(1))
    ...
    def _select_operators_v2(self, operators: List[Dict], num_operators: int = 3) -> List[int]:
        """Select safer operators for placeholder templates.

        Generic templates often look like OPERATOR1(DATA_FIELD1).
        Therefore we should avoid selecting binary arithmetic operators such as add,
        subtract, multiply and divide as generic OPERATOR placeholders.
        """
        import random
        ...
        forbidden_ops = {
            # Invalid ts_* operators generated by old repair logic
            "ts_add",
            "ts_subtract",
            "ts_multiply",
            "ts_divide",
            ...
            # Group operators need group/category arguments, unsafe for generic placeholders
            "group_scale",
            "group_rank",
            "group_zscore",
            "group_neutralize",
            ...
            # Reduce operators usually need special input structures / extra arguments
            "reduce_choose",
            "reduce_kth",
            "reduce_max",
            "reduce_min",
            ...
            # Other complex operators not suitable for simple OPERATOR(DATA_FIELD) templates
            "bucket",
            "trade_when",
        }

        # These need at least 2 inputs and often break templates like OPERATOR1(DATA_FIELD1)
        ...
        for idx, op in enumerate(operators):
            name = self._operator_name(op).lower()
            ...
            if name in binary_arithmetic_ops:
                continue

            input_count = self._operator_input_count(op)

            # If metadata clearly says this operator needs >1 inputs, do not use it
            # for generic OPERATOR placeholders.
            if input_count and input_count > 1:
                continue

            safe_indices.append(idx)

            if name in preferred_unary_ops:
                preferred_indices.append(idx)

        candidate_indices = preferred_indices if len(preferred_indices) >= num_operators else safe_indices

        if not candidate_indices:
            logger.warning(
                "Safe operator filter removed all operators; falling back to original operator pool."
            )
            candidate_indices = list(range(len(operators)))

        num_operators = min(num_operators, len(candidate_indices))
        selected_indices = random.sample(candidate_indices, num_operators)

        logger.debug(
            f"Selected safe operator indices: {selected_indices} "
            f"(safe={len(safe_indices)}, preferred={len(preferred_indices)}, total={len(operators)})"
        )

        return selected_indices
    
    def _replace_field_placeholders(self, template: str, selected_fields: List[Dict], region: str = None) -> str:
        """Replace DATA_FIELD1, DATA_FIELD2, etc. with actual safe field IDs.

        This is the final gate before a template becomes a real expression.  It
        therefore checks both API metadata and DB-learned event-input fields.
        """
        import re
        ...
        db_event_input_fields = self._load_db_event_input_fields(region=region, delay=1)
        safe_selected_fields = []

        for field in selected_fields:
            field_id = self._field_id(field)
            if self._is_safe_data_field(field, db_event_input_fields=db_event_input_fields):
                safe_selected_fields.append(field)
            else:
                logger.warning(
                    f"Skipping unsafe/event field during placeholder replacement: {field_id}"
                )

        # If all selected fields are unsafe, fetch a fresh safe pool for the region.
        if not safe_selected_fields and region:
            try:
                all_fields = self.get_data_fields_for_region(region)
                safe_selected_fields = self._filter_safe_data_fields(all_fields, region=region, delay=1)
                logger.warning(
                    f"Recovered {len(safe_selected_fields)} safe fields from full region field pool for {region}"
                )
            except Exception as e:
                logger.warning(f"Could not recover safe fields for region {region}: {e}")

        if not safe_selected_fields:
            logger.warning(
                "No safe fields available during placeholder replacement; keeping original template."
            )
            return template

        field_mapping = {}

        for i, field in enumerate(safe_selected_fields[:10], start=1):
            field_id = self._field_id(field)
            if field_id:
        ...
        if replacements_made:
            logger.info(
                f"?Safe field placeholder replacement ({len(replacements_made)} replacements): "
                f"{template[:50]}... -> {result[:50]}..."
            )
