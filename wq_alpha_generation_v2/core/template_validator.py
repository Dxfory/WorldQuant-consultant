...
FIXED EXPRESSION:"""
    ...
    def _extract_candidate_field_ids(self, template: str) -> Set[str]:
        """Extract likely data field ids from a FASTEXPR template.

        Operators and FASTEXPR keywords are excluded.  The purpose is not to
        fully parse the expression; it is to learn which fields caused an
        event-input error so future generation can avoid them.
        """
        if not template:
            return set()

        tokens = re.findall(r"\b[a-z][a-z0-9_]{3,}\b", str(template).lower())

        operator_names = {
            str(op.get("name", "")).lower()
            for op in (self.operators or [])
            if isinstance(op, dict) and op.get("name")
        }
        common_keywords = {
            "add", "subtract", "multiply", "divide", "power", "signed_power",
            "abs", "log", "sqrt", "inverse", "min", "max", "rank", "zscore",
            "winsorize", "scale", "sign", "reverse", "densify", "if_else",
            "and", "or", "not", "true", "false", "nan", "filter",
            "to_nan", "is_nan", "pasteurize", "group_neutralize",
            "group_rank", "group_zscore", "group_mean", "group_backfill",
        }

        fields = set()
        for token in tokens:
            if token in operator_names or token in common_keywords:
                continue
            if token.startswith("ts_") or token.startswith("vec_") or token.startswith("group_"):
                continue
            # Most WQB field ids contain underscores and are longer than
            # ordinary FASTEXPR keywords.
            if "_" in token and len(token) >= 6:
                fields.add(token)

        return fields

    def _mark_event_input_fields_from_template(self, template: str, reason: str = "") -> Set[str]:
        """Mark fields from a failed event-input expression as event-input fields.

        This is conservative, but it prevents the miner from repeatedly using
        fields that WorldQuant Brain already indicated are incompatible with
        ordinary arithmetic operators.
        """
        fields = self._extract_candidate_field_ids(template)
        if not fields:
            return set()
        ...
        try:
            ...
            if "field_id" not in cols or "is_event_input" not in cols:
                conn.close()
                return set()
            ...
            for field_id in fields:
                cursor.execute(
                    """
                    UPDATE field_types
                    SET is_event_input = 1,
                        field_type = CASE
                            WHEN field_type IS NULL OR field_type = '' THEN 'EVENT_LEARNED'
                            ELSE field_type
                        END,
                        updated_at = CURRENT_TIMESTAMP
                    WHERE LOWER(field_id) = ?
                    """,
                    (field_id.lower(),),
                )

                if cursor.rowcount == 0:
                    # Keep insert schema aligned with BacktestStorage's field_types table.
                    try:
                        cursor.execute(
                            """
                        ...
                        # Some older schemas may not support full insertion; the
                        # UPDATE path above is the main compatibility path.
                        pass
            ...
            if marked:
                logger.warning(
                    f"Marked {len(marked)} field(s) as event input from simulation error: "
                    f"{sorted(marked)[:5]}"
                )
            ...
            logger.debug(f"Failed to mark event-input fields from template: {e}")
            return set()

    def _get_event_input_fields(self, region: str, delay: int = 1) -> Set[str]:
        """Get set of event/vector field IDs for a region and delay.

        This method is schema-tolerant and also reads globally learned event
        fields.  Returning lower-case ids makes comparisons stable.
        """
        try:
            from ..storage.backtest_storage import BacktestStorage
            storage = BacktestStorage(self.db_path)
            ...
            if "field_type" in cols:
                where_parts.append("LOWER(COALESCE(field_type, '')) IN ('event', 'events', 'vector', 'vectors', 'event_learned')")
            if "category" in cols:
    ...
    def _aggressive_event_input_fix(self, template: str, error_message: str, region: str = None) -> str:
        """Disable unsafe event-input auto-repair.

        The old implementation converted ordinary operators into invented or
        unsupported ts_* variants such as ts_add/ts_log/ts_sqrt.  That created
        a failure loop.  We keep the method for compatibility, but it now
        returns the original template and lets the field filters prevent future
        event-input selections.
        """
        logger.warning(
            "Event input auto-repair disabled: not replacing operators with invalid ts_* variants."
        )
        return template

    def _fix_event_input_error(self, template: str, error_message: str, region: str = None) -> Tuple[str, List[str]]:
        """Handle event-input errors without inventing ts_* replacements.

        Event/vector fields require dedicated handling.  Generic arithmetic
        substitutions such as add -> ts_add are unsafe because those operators
        may not exist or may have different signatures.  The correct repair path
        for this miner is to learn the problematic field/operator and avoid that
        field in future generated templates.
        """
        fixes = []

        operator_match = re.search(
            r'Operator\s+(\w+)\s+(?:does not support|expects only)\s+event inputs',
            error_message or "",
            re.IGNORECASE,
        )
        if not operator_match:
            operator_match = re.search(
                r'(\w+)\s+(?:does not support|expects only)\s+event inputs',
                error_message or "",
                re.IGNORECASE,
            )

        if operator_match:
            problematic_operator = operator_match.group(1).lower()
            logger.warning(
                f"Skipping event-input operator replacement for {problematic_operator}; "
                f"no safe generic replacement is available."
            )
            fixes.append(
                f"Skipped unsafe event-input replacement for {problematic_operator}"
            )

        marked_fields = self._mark_event_input_fields_from_template(
            template,
            reason=error_message or "event input error",
        )
        if marked_fields:
            fixes.append(
                f"Marked event-input field(s) to avoid later: {', '.join(sorted(marked_fields)[:5])}"
            )
    ...
    def _learn_event_input_compatibility(self, template: str, error_message: str):
        ...
        marked_event_fields = self._mark_event_input_fields_from_template(template, reason=error_message)
        if marked_event_fields:
            logger.info(f"📚 Learned event-input fields: {sorted(marked_event_fields)[:5]}")
        ...
        if operator_match:
            ...
            try:
                ...
                if hasattr(self, '_fix_event_input_error'):
                    ...
                    if replacement in bad_replacement_operators:
                        logger.warning(
                            f"Blocked invalid compiler knowledge replacement: "
                            f"{operator_name} -> {replacement}"
                        )
                        replacement = None                
    ...
    def _fix_input_count_error(self, template: str, error_message: str) -> Tuple[str, List[str]]:
        ...
        if expected is not None and actual is not None:
            ...
            # Only these operators are safe to reduce to one input.
            # Do NOT reduce add/subtract/multiply/divide to one input.
            unary_input_fix_allowlist = {
                "log",
                "sqrt",
                "abs",
                "reverse",
            ...
            for op_match in operators:
                ...
                # If the error says "should be exactly 1", only fix true unary operators.
                # This prevents wrongly changing add(x, y) into add(x).
                if expected == 1 and op_name_l not in unary_input_fix_allowlist:
                    logger.debug(
                        f"Skipping input-count reduction for non-unary operator: {op_name}"
                    )
                    continue

                # Never reduce binary arithmetic operators to fewer than 2 inputs.
                if op_name_l in binary_operators and expected < 2:
                    logger.warning(
                        f"Refusing to reduce binary operator {op_name} to {expected} input(s)"
                    )
                    continue
